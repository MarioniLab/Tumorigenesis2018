...
anno_full <- read.csv("../data/firstBatch_Robjects/Cluster_final.csv")
pD.sub <- left_join(anno_full,pD)
pD.sub$Class <- ifelse(pD.sub$ClusterLabels %in% c("C4","C12","C19","C14"),"Epithelial","Non-Epi")
summ <- group_by(pD.sub, Condition) %>% 
    summarize(scRNA_Epithelial=(sum(Class=="Epithelial")/n())*100,
	      scRNA_Basal=(sum((ClusterLabels=="C12"))/sum(Class=="Epithelial"))*100,
	      scRNA_Luminal=(sum((ClusterLabels %in% c("C19","C14")))/sum(Class=="Epithelial"))*100)
...
pltlist <- list()
for (cond in conds) {
    tsn <- read.csv(paste0("../data/firstBatch_Robjects/tSNE_",cond,"_raw.csv"))
...
```{r, fig.width=12, fig.height=12, warning=FALSE, message=FALSE}
tsn <- read.csv("../data/firstBatch_Robjects/tSNE_nontumor_raw.csv")
pD.sub <- right_join(pD,tsn)
anno <- read.csv("../data/firstBatch_Robjects/CellTypes.csv")
...
```{r, fig.width=12, fig.height=12, warning=FALSE, message=FALSE}
substructure <- read.csv("../data/firstBatch_Robjects/Substructure.csv")
...
dms <- read.csv("../data/firstBatch_Robjects/DiffusionMap.csv")
...
anno_full <- read.csv("../data/firstBatch_Robjects/Cluster_all.csv")[,-1]
...
## UMAP
```{r, fig.width=12, fig.height=12, warning=FALSE, message=FALSE}
umap <- read.csv("../data/firstBatch_Robjects/UMAP_all.csv")
...
```{r, fig.width=12, fig.height=12, warning=FALSE, message=FALSE}
dms <- read.csv("../data/firstBatch_Robjects/DiffusionMap_Epithelium.csv")
