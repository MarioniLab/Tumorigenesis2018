...
genes <- unique(genes)
library(viridis)
bubblePlot(m, genes, pD$CellType ,angled=FALSE) %+% coord_flip() %+% scale_color_viridis(option="C")
ggsave("MyeloidExpression.svg",width=10, height=3.5)
```
...
library(ggrepel)
## Option 1
cells <- pD$barcode[pD$CellType %in% c("Mp1","Mp2")]
pD.sub <- pD[cells,]
m.sub <- m[,cells]
m.sub <- m.sub[rowMeans(m.sub)>0.01,]

test <- findMarkers(m.sub,clusters=factor(pD.sub$CellType),
		    block=factor(pD.sub$Sample),full.stats=TRUE, lfc=0.5)
res <- data.frame(test[[1]][[4]])
res$Gene <- rownames(res)
volcano <- ggplot(res, aes(x=logFC,y=-log.FDR)) +
    geom_point(size=1) + 
    geom_hline(yintercept=2,lty="dashed") +
    geom_vline(xintercept=log2(1.2),lty="dashed") +
    geom_vline(xintercept=-log2(1.2),lty="dashed") +
    geom_text_repel(data=res[1:30,], aes(x=logFC,y=-log.FDR,label=Gene)) +
    ggtitle("Mp2vsMp1")

ggsave("Volcano.svg",height=7,width=7)
...
overviewPlot(m,pD,"Ly6a",bplot=FALSE) + scale_color_viridis(option="C")
ggsave("Ly6a.pdf")
overviewPlot(m,pD,"Cx3cr1",bplot=FALSE) + scale_color_viridis(option="C")
ggsave("Cx3cr1.pdf")
overviewPlot(m,pD,"Ms4a7",bplot=FALSE) + scale_color_viridis(option="C")
ggsave("Ms4a7.pdf")
```
...
```{r, warning=FALSE, message=FALSE, echo=FALSE}
pD.sub <- pD[pD$CellType %in% c("Dc209","DcPot","mDc"),]
m.sub <- m[,pD.sub$barcode]
...
plotlist <- list()
exps <- data.frame(gene=m.sub["Ccr7",],
		    barcode=colnames(m.sub))
colnames(exps)[1] <- "Ccr7"
fp <- left_join(pD.sub,exps)
fp$Ccr7 <- scale(fp$Ccr7)
for (i in seq_along(unique(fp$ranktumorTime))) {
    plotlist[[i]] <- ggplot(fp, aes(x=DC1, y=DC2)) +
	geom_point(color="grey80") +
	theme(axis.ticks=element_blank(),axis.text=element_blank()) +
	ylab("") +
	xlab("") + 
	geom_point(data=fp[fp$ranktumorTime==i,], size=2.5, pch=21, aes(x=DC1, y=DC2, fill=Ccr7)) +
	scale_fill_viridis(option="C",begin=0,end=max(fp[fp$ranktumorTime==i,"Ccr7"])/max(fp$Ccr7)) +
	theme(legend.position="none")
}

plot_grid(plotlist=plotlist,nrow=1)
ggsave("DendCells.svg",width=20,height=2.5)
