...
ggplot(top, aes(x=logFC, y=-log10(FDR))) +
    ...
    geom_point(data=top[top$FDR<0.01,], size=.9)+
    geom_label_repel(data=top[top$logFC>5 & top$FDR < 0.01,], aes(label=Gene)) +
...
ggplot(top, aes(x=logFC, y=-log10(FDR))) +
    geom_point(size=.5,color="grey80") +
    geom_point(data=top[top$FDR<0.01,], size=.9)+
    geom_label_repel(data=top[top$FDR < 0.01,][1:20,], aes(label=Gene)) +
    geom_hline(yintercept=2) +
    ggtitle("DE w time")
