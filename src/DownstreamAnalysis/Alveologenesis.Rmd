...
pltUMAP <- function(pD.sub,pD) {
	...
	geom_point_rast(color="grey80",size=.5,dpi=dpi) +
	geom_point_rast(data=pD.sub, aes(color=CellTypes), size=.5,dpi=dpi) +
	scale_color_manual(values=levels(pD.sub$Colors)) +
	theme_void() +
...
pD4 <- pD[pD$Condition=="4.5dG",]
pD9 <- pD[pD$Condition=="9.5dG",]
pD14 <- pD[pD$Condition=="14.5dG",]
...
ggsave(paste0(outdr,"UMAP_4dG.png"),pltUMAP(pD4,pD),dpi=dpi)
ggsave(paste0(outdr,"UMAP_9dG.png"),pltUMAP(pD9,pD),dpi=dpi)
ggsave(paste0(outdr,"UMAP_14dG.png"),pltUMAP(pD14,pD),dpi=dpi)



# Some GEX
pltGEXUMAP <- function(pD,gene) {
    p <- ggplot(pD, aes(x=UMAP1, y=UMAP2, color=gene)) +
	geom_point_rast(size=.5,dpi=dpi) +
	theme_void() +
	scale_color_viridis() +
	theme(legend.position="none")
    return(p)
...
pki67 <- plot_hexbin_gene(sce.sub, gene="Mki67", type="logcounts",
		 action="mean", xlab="DC1", ylab="DC2") +
theme(text=element_text(size=14))

pCsn2 <- plot_hexbin_gene(sce.sub, gene="Csn2", type="logcounts",
		 action="mean", xlab="DC1", ylab="DC2") +
theme(text=element_text(size=14))

pAldh1a3 <- plot_hexbin_gene(sce.sub, gene="Aldh1a3", type="logcounts",
		 action="mean", xlab="DC1", ylab="DC2") +
theme(text=element_text(size=14))

pWap <- plot_hexbin_gene(sce.sub, gene="Wap", type="logcounts",
		 action="mean", xlab="DC1", ylab="DC2") +
theme(text=element_text(size=14))


ggsave(paste0(outdr,"DiffMap_Ki67.pdf"),pki67)
ggsave(paste0(outdr,"DiffMap_Csn2.pdf"),pCsn2)
ggsave(paste0(outdr,"DiffMap_Aldh1a3.pdf"),pAldh1a3)
ggsave(paste0(outdr,"DiffMap_Wap.pdf"),pWap)
```
