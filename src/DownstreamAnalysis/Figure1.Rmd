...
ptime$ptimeBin[ptime$Condition %in% c("WKBR75.4bTM","WKBR61.4dTM")] <- 5
##
...
p1 <- ggplot(pcs.all, aes(x=PC1, y=PC2,fill=Age)) +
    geom_point(size=4,pch=21) +
    scale_fill_manual(values=colrd) +
...
# X <- model.matrix(~0+pD$Groups)
# colnames(X) <- levels(pD$Groups)
# X <- t(t(X)/colSums(X))
...
barplt <- ggplot(sumry, aes(x=factor(ptimeBin,levels=c(6,5,4,3,2,1)), y=frac, fill=CellTypes)) +
...
pltUMAP <- function(pD.sub,pD) {
	...
	geom_point(data=pD.sub, aes(color=Groups), size=.1) +
...
ggsave(paste0(outdr,"UMAP_Bin_1.svg"),pltUMAP(pD1,pD))
ggsave(paste0(outdr,"UMAP_Bin_2.svg"),pltUMAP(pD2,pD))
ggsave(paste0(outdr,"UMAP_Bin_3.svg"),pltUMAP(pD3,pD))
ggsave(paste0(outdr,"UMAP_Bin_4.svg"),pltUMAP(pD4,pD))
ggsave(paste0(outdr,"UMAP_Bin_5.svg"),pltUMAP(pD5,pD))
ggsave(paste0(outdr,"UMAP_Bin_WT_Young.svg"),pltUMAP(pDWtY,pD))
ggsave(paste0(outdr,"UMAP_Bin_WT_Old.svg"),pltUMAP(pDWtO,pD))
...
pbox <- ggplot(ptime, aes(x=ptimeBin, y=Age)) +
    geom_boxplot()
pbox
```
...
# pD <- pD[pD$MajorGroups %in% "Fibroblast",]
pD <- droplevels(pD)
sumry <- group_by(pD, ptimeBin, Groups) %>%
    summarize(n=n()) %>%
    mutate(frac=n/sum(n)) %>%
    ungroup() # %>%
#     mutate(cumfrac=cumsum(frac)) %>%
...
comb <- table(sumry$Groups,sumry$ptimeBin) 
missing.comb <- which(comb==0, arr.ind=TRUE)
if (nrow(missing.comb)>0) {
...
sumry$Colors <- plyr::mapvalues(as.character(sumry$Groups),
				...
				c("#E0ECF7","#37A055","#3787C0","#FDCFBB","#ABCFE5",
				  "#FEEDDB","#737373"))

sumry <- arrange(sumry, ptimeBin, Groups) %>%
    group_by(ptimeBin) %>%
    mutate(cumfrac=cumsum(frac)) %>%
    ungroup()

p5 <- ggplot(sumry, aes(x=as.numeric(ptimeBin), ymin=cumfrac-frac, ymax=cumfrac,
		  fill=Groups)) +
    geom_ribbon() +
    scale_fill_manual(values=unique(sumry$Colors)) +
    xlab("Tumor Development") +
...
```{r, message=FALSE, warning=FALSE, fig.width=10, fig.height=7,eval=FALSE}
library(dplyr)
# Ribbon plot
pD <- data.frame(colData(sce))
# pD <- left_join(pD,pcs.all[,c("Condition","ptime")])
...
# pD <- pD[pD$MajorGroups %in% "Fibroblast",]
pD <- droplevels(pD)
sumry <- group_by(pD, ptimeBin, MajorGroups) %>%
    summarize(n=n()) %>%
    mutate(frac=n/sum(n)) %>%
    ungroup() # %>%
#     mutate(cumfrac=cumsum(frac)) %>%
...
comb <- table(sumry$MajorGroups,sumry$ptimeBin) 
missing.comb <- which(comb==0, arr.ind=TRUE)
if (nrow(missing.comb)>0) {
    add.missing <- data.frame("MajorGroups"=rownames(missing.comb),
		      "ptimeBin"=colnames(comb)[missing.comb[,"col"]],
		      "frac"=0,
		      "n"=0)
    sumry <- rbind(sumry,add.missing)
...
sumry$Colors <- plyr::mapvalues(as.character(sumry$MajorGroups),
				as.character(unique(pD$MajorGroups)),
				c("#084D96","#1B843F","#FDCFBB","#737373"))

sumry <- arrange(sumry, ptimeBin, MajorGroups) %>%
    group_by(ptimeBin) %>%
    mutate(cumfrac=cumsum(frac)) %>%
    ungroup()

p6 <- ggplot(sumry, aes(x=as.numeric(ptimeBin), ymin=cumfrac-frac, ymax=cumfrac,
		  fill=MajorGroups)) +
    geom_ribbon() +
    scale_fill_manual(values=unique(sumry$Colors)) +
    xlab("Tumor Development") +
