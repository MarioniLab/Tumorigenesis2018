...
```{r, message=FALSE, warning=FALSE, fig.width=9, fig.height=9}
library(wesanderson)
ggplot(pD, aes(x=UMAP1, y=UMAP2, color=Walktrap6)) +
    geom_point() +
    scale_color_manual(values=wes_palette("FantasticFox1",length(unique(pD$Walktrap6)),type="continuous")) 
```
...
# ratio <- clusterModularity(igr.cor, pD$Walktrap4, as.ratio=TRUE)
# library(pheatmap)
...
```{r, message=FALSE, warning=FALSE, fig.width=9, fig.height=9, cache=TRUE}
# ratio <- clusterModularity(snn, pD$Walktrap6, as.ratio=TRUE)
# library(pheatmap)
# pheatmap(log2(ratio+1), cluster_rows=FALSE, cluster_cols=FALSE,
#          color=colorRampPalette(c("white", "blue"))(100))
```
...
conf <- as.matrix(table(pD.sub$CellType,pD.sub$Walktrap5))
conf <- t(t(conf)/colSums(conf))
library(pheatmap)
pheatmap(conf,
	 cluster_rows=FALSE,
...
conf <- as.matrix(table(pD.sub$CellType,pD.sub$Walktrap6))
conf <- t(t(conf)/colSums(conf))
library(pheatmap)
pheatmap(conf,
	 cluster_rows=FALSE,
...
### UMAP Walktrap
```{r, message=FALSE, fig.width=8, fig.height=8}
pD.my <- pD[pD$UMAP1>-5 & pD$UMAP1<3 & pD$UMAP2<7 & pD$UMAP2>2.5,]
...
### SNN Louvain
```{r, message=FALSE, fig.width=8, fig.height=8}
p0 <- ggplot(pD.my, aes(x=UMAP1, y=UMAP2, color=Walktrap6)) +
    geom_point() +
    scale_color_manual(values=wes_palette("FantasticFox1",length(unique(pD.my$Walktrap6)),type="continuous")) 
