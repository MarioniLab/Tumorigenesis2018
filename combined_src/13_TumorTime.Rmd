...
getPC <- function(sce, getPCObject=FALSE) {
    ...
    # 
    cor.logcpm <- removeBatchEffect(as.matrix(logcpm), batch=sce.sum$Batch)
...
pcs.epi$ptime <- (pcs.epi$PC1-min(pcs.epi$PC1))/max(pcs.epi$PC1-min(pcs.epi$PC1)) * 100

sumry <- left_join(sumry,pcs.epi[,c("Condition","ptime")])
sumry$Condition <- factor(sumry$Condition,
			  levels=sumry$Condition)
...
```{r, message=FALSE, warning=FALSE}
cr <- round(cor(as.numeric(pcs.epi$Age), pcs.epi$PC1),2)

ggplot(pcs.epi, aes(x=PC1, y=as.numeric(Age))) +
    geom_point() + 
    xlab("PC1") +
    ylab("Age [w]") 
...
sce.sum <- aggregateAcrossCells(sce.full[,sce.full$MajorGroups=="Epithelial"],
		      id=DataFrame(Condition=sce.full[,sce.full$MajorGroups=="Epithelial"]$Condition),
		      use_exprs_values="counts",
		      average=FALSE)
...
# 
colnames(cor.logcpm) <- pD.sum$Condition
...
add <- data.frame("Condition"=names(cors),
		  "CorToWT"=cors)
pcs.epi <- left_join(pcs.epi, add)

ggplot(pcs.epi, aes(x=PC1, y=CorToWT)) +
    geom_point() 
```
...
```{r, message=FALSE, warning=FALSE}
dmba.data <- readRDS("../data/combined_Robjects/DMBA_Bulk.rds")
pDat <- dmba.data[["pDat"]]
...
sce.sum <- aggregateAcrossCells(sce.epi,
		  id=DataFrame(Condition=sce.epi$Condition),
...
dmba.counts <- dmba.counts[rownames(cor.logcpm),]
# Set up DGEList
...
```{r, message=FALSE, warning=FALSE}
pcs <- getPC(sce.epi, getPCObject=TRUE)
...
```{r, message=FALSE, warning=FALSE}
outvr <- data.frame()
for (ptm in pcs.epi$ptime) {
    cond <- pcs.epi$Condition[pcs.epi$ptime==ptm]
...
graphs <- list()
smps <- pcs.epi$ptime[pcs.epi$ptime<90]
for (ptm in smps) {
    cond <- pcs.epi$Condition[pcs.epi$ptime==ptm]
    cells <- colnames(sce)[sce$Condition==cond]
    sce.sub <- sce[,cells]
...
mnncor <- readRDS("../data/combined_Robjects/CorrectedPCA.rds")
