...
rm(dataList)
cluster <- read.csv(file="../data/combined_Robjects/Clusters.csv",stringsAsFactors=FALSE)[,-1]
cluster$Cluster <- as.factor(paste0("C",cluster$Cluster))
...
compCluster <-  function(pcs, cluster, m, ...) {
	...
	cs <- paste0("C",cluster,"S", finalSubclusters)
...
freq.tab <- table(pD$Cluster)
# clusters <- names(freq.tab)[freq.tab>200]
clusts <- unique(pD$Cluster)
## remove later
set.seed(20)
# cluster <- sample(clusters, 5)
