...
# Read in data
ddata <- readRDS("../data/combined_Robjects/DoubletData.rds")
...
# Visualizing the alternative doublet scores
score_plots <- lapply(names(ddata), function(nm) {
		   x <- ddata[[nm]]
		   knn <- x$KNNs
		   outlrs <- scater::isOutlier(x$DbltScore,type="higher",log=FALSE)
		   x$nk <- apply(knn,1, function(rw) sum(outlrs[rw]))
		   x <- data.frame(x)
		   x$nk <- x$nk > 3
		   totdob <- sum(x$nk)
		   x <- dplyr::arrange(x,nk)
		   p <- ggplot(x, aes(x=SubUMAP1, y=SubUMAP2, color=nk)) +
			       geom_point(size=0.9) +
			       #                                scale_color_viridis() +
			       theme(legend.position="none", panel.background = element_rect(colour = "black", linetype = 1, size = 0.5),
				     axis.line = element_blank(),
				     ...
				     axis.text = element_blank(),
				     axis.title = element_blank(),) +
			       ggtitle(paste0(nm," (n=",nrow(x),", k=",length(unique(x$Cluster))," ndb=",totdob,")")) 
			   return(p)
})
names(score_plots) <- names(ddata)
plot_grid(plotlist=score_plots)
...
```{r, message=FALSE}
pD <- colData(readRDS("../data/combined_Robjects/SCE_QC_norm.rds"))
pD <- data.frame(pD)
library(dplyr)
...
# Read in BC PCA
sce.cor <- readRDS("../data/combined_Robjects/Corrected_preDoublet.rds")
...
```{r, message=FALSE, warning=FALSE}
m <- logcounts(readRDS("../data/combined_Robjects/SCE_QC_norm.rds"))
...
# Save
```{r, message=FALSE, warning=FALSE}
# ddata$isRbc <- ddatadf$barcode %in% rbcs
